    ...
    if (terms.length > 1) {
      ...
      for (let i = 0; i < terms.length; i++) {
        try {
          ...
          const termParams = { ...params, query: terms[i]! };
          if (isCountrySearch) {
            const termListings = await scrapeCountry(termParams, search.country!, search.userId);
            allListings.push(...termListings);
          } else {
            const result = await searchMarketPlace(termParams, search.userId);
            allListings.push(...result.listings);
          }
        } catch (error) {
          logger.error(`[runSearch] Multi-query term "${terms[i]}" failed, continuing with remaining terms:`, error);
        }
        if (i < terms.length - 1) {
          await delay(DEFAULT_PAGE_DELAY_MS);
        }
